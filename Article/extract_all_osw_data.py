...
# Get ALL OSW files including _fixed versions
osw_files = sorted(glob.glob("results/openms_output/openswath/*.osw"))
osw_files = [f for f in osw_files if not f.endswith("-journal")]
...
for i, osw_file in enumerate(osw_files):
    ...
    file_size = os.path.getsize(osw_file) / (1024**3)  # GB
    
    print(f"[{i+1:2d}/{len(osw_files)}] {sample_name} ({file_size:.1f}GB)")
...
print(f"\nFile size summary: {large_files} large (>1GB), {small_files} small")

if all_data:
    combined_df = pd.concat(all_data, ignore_index=True)
    print(f"\nTotal proteins: {combined_df['PROTEIN_ACCESSION'].nunique()}")
    print(f"Total samples: {combined_df['sample'].nunique()}")
    print(f"Sample list: {sorted(combined_df['sample'].unique())}")
    ...
    # Save
    matrix.to_csv("results/protein_matrix_openms_complete.csv")
    print(f"\n🎉 COMPLETE ANALYSIS: {matrix.shape[0]} proteins × {matrix.shape[1]} samples")
