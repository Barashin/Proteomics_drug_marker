...
> **注意:** jPOSTリポジトリに公開されているmzMLファイルは**CRC04以降の一部のみ**で、CRC01〜03のmzMLは含まれていません。論文には特にこの点についての記述はありません。本シリーズでは公共データベースで公開されているmzMLファイルをそのまま使用して解析を進めます。すべてのRAWファイルからmzMLに変換して全16患者分で解析を行いたい方は、[#3 RAW→mzML変換](article-03-convert.md)を参照してください。
