---
title: "Sage解析での主成分分析（PCA）と論文比較【論文再現シリーズ #7b】"
emoji: "🧠"
type: "tech"
topics: ["proteomics", "python", "pca", "sage", "labcode"]
published: false
---

# Sage解析での主成分分析（PCA）と論文比較

## はじめに

前回（[#7a Sage基本可視化](article-07a-visualization-basics.md)）では、相関行列とクラスタリングにより基本的な群構造を確認しました。この記事では、主成分分析（PCA）による高次元データの次元削減解析を行い、腫瘍/正常組織の分離状況をより詳細に評価します。また、Toyota et al. 2025論文との数値比較も実施します。

> **📝 INFO**
>
**この記事で行う処理**
Sage-proteomicsで検出した2,081タンパク質のデータに対してPCA（主成分分析）を実行し、2次元の主成分空間でNormal/Tumorの群分離を可視化します。各主成分の寄与率を計算し、論文値と比較することで、商用利用可能なsage-proteomicsの妥当性を定量的に評価します。

## 前提

- [#7a Sage基本可視化](article-07a-visualization-basics.md) が完了していること
- 前処理済みデータが利用可能
...
```bash
python scripts/step_07b_visualization_pca.py
```
...
**PC1 寄与率が論文の 42.1% に対して本実装 39.7% と近い値**を示す点が重要です。これは「大腸がんの Tumor/Normal 対比が強い生物学的シグナル」を示し、ツール（DIA-NN → sage）を変えても **本質的な群構造は再現される** ことを意味します。

### Sage-proteomicsの商用利用価値

1. **生物学的シグナルの保持**: PC1寄与率がほぼ同等（39.7% vs 42.1%）
2. **群分離の明確性**: Normal/Tumor が明瞭に分離
3. **商用利用可能**: MIT ライセンスで企業応用可能
4. **検出数は論文の1/5**: 2,081 vs 10,329タンパク質だが、主要シグナルは維持

検出タンパク質数は論文の約1/5ですが、**主要な生物学的シグナル（PC1寄与率）がほぼ維持**されていることは、sage-proteomicsが**コストパフォーマンスに優れた代替手法**であることを示しています。

## まとめ

PCA解析により、Sage解析結果の生物学的妥当性を定量的に確認しました：

### 主要成果

1. **PC1寄与率39.7%**: 論文値（42.1%）に近い値で生物学的シグナルの再現を確認
2. **明確な群分離**: Normal/Tumor が主成分空間で明瞭に分離
3. **商用利用可能**: MIT ライセンスで企業応用に制約なし
4. **コストパフォーマンス**: 検出数は1/5だが主要シグナルを維持

Sage-proteomicsは、DIA-NNに比べて検出タンパク質数は少ないものの、生物学的に重要なシグナルを保持しており、**商用利用可能な実用的代替手法**として高い価値を示しました。

> 前回: [#7a Sage基本可視化](article-07a-visualization-basics.md)
> 次回: [#8a 差分発現解析](article-08a-differential-volcano.md) — Welch's t-testとVolcanoプロット

#バイオインフォマティクス #プロテオミクス #PCA #統計解析 #Sage #labcode