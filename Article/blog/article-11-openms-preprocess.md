---
title: "深層学習DIA解析結果を前処理する【論文再現シリーズ #11】"
emoji: "🧠"
type: "tech"
topics: ["proteomics", "openms", "python", "bioinformatics", "labcode"]
published: false
---
...
OpenMS + AlphaPeptDeepで深層学習を使って生成した**19,981タンパク質のマトリクス**を、統計解析に適した形に前処理します。この記事では、Log2変換・欠損値フィルタリング・欠損値補完をPythonで実装し、Sageの結果（#6）と同じ形式の前処理済みデータを作成します。

> **📝 INFO**
>
**この記事で行う処理**
OpenMS + AlphaPeptDeepで検出した19,981タンパク質の生強度値を統計解析用に前処理します。①Log2変換で正規分布に近づけ、②有効値が少なすぎるタンパク質を除去し（70%ルール）、③検出限界以下で欠損になった値をPerseus互換のdownshift法で補完します。出力される`preprocessed_data_openms.csv`は、以降の統計検定や可視化（#12-15）で使用されます。


## 前提

- [#1 環境構築](article-01-setup.md) が完了していること
- [#10 OpenMS + AlphaPeptDeep深層学習DIA解析](article-10-openms.md) で `results/protein_matrix_from_openms.csv` が生成されていること
- **対応Notebook**: [`notebooks/step_11_openms_preprocess.ipynb`](../notebooks/step_11_openms_preprocess.ipynb) — この記事のコードをセルごとに実行できます

## 本書で扱うデータ規模

本記事では **OpenMS + AlphaPeptDepで検出した19,981タンパク質**を前処理します。深層学習ベースの同定により、Sage（2,110タンパク質）の**9.5倍の検出性能**を実現しています：
...
df.to_csv(os.path.join(RESULTS_DIR, "preprocessed_data_openms.csv"))
...
print(f"  → preprocessed_data_openms.csv")
...
> 次回: [#12a 深層学習DIA基本可視化](article-12a-openms-visualization-basics.md) — 相関行列・クラスタリング
