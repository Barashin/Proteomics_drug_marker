---
title: "深層学習DIA解析結果の基本可視化：相関行列・クラスタリング【論文再現シリーズ #12a】"
emoji: "📊"
type: "tech"
topics: ["proteomics", "openms", "visualization", "correlation", "labcode"]
published: false
---

# 深層学習DIA解析結果の基本可視化：相関行列・クラスタリング

## はじめに

[#11 深層学習結果の前処理](article-11-openms-preprocess.md)で得られた**19,981タンパク質の前処理済みデータ**を基本的な可視化手法で解析し、腫瘍組織と正常組織の発現パターンの違いを確認します。この記事では、相関行列ヒートマップと階層的クラスタリングによる基本的な群構造の可視化を行います。

> **📝 INFO**
>
**この記事で行う処理**
OpenMS + AlphaPeptDeepで検出した19,981タンパク質の前処理済みデータを、相関行列ヒートマップと階層的クラスタリングで可視化し、NormalとTumorの群構造を確認します。深層学習による大幅な検出数増加（9.5倍）が、基本的な可視化でも明確な群分離として現れるかを検証します。

## 前提

- [#11 深層学習結果の前処理](article-11-openms-preprocess.md) が完了していること
- `results/preprocessed_data_openms.csv` が存在
- **対応Notebook**: [`notebooks/step_12_openms_visualization.ipynb`](../notebooks/step_12_openms_visualization.ipynb) — この記事のコードをセルごとに実行できます
...
## コード全文（対応Notebook: step_12_openms_visualization.ipynb）
...
```bash
python scripts/step_12a_openms_visualization_basics.py
```

**Notebook でセルごとに実行する場合:**

```bash
jupyter notebook notebooks/step_12_openms_visualization.ipynb
```
...
全19,981タンパク質の発現量をもとに、Ward法による階層的クラスタリングを行いました。デンドログラム（樹形図）を見ると、NormalサンプルとTumorサンプルが明確に2つの枝に分離しています。教師なし手法（ラベルを使わない分類）でも、腫瘍/正常の群構造を正しく再現できることが確認されました。

## まとめ

相関行列ヒートマップと階層的クラスタリングの両手法で、深層学習DIA解析結果における腫瘍組織と正常組織の明確な分離を確認しました。19,981タンパク質という大規模データセットでも、基本的な可視化手法により生物学的に妥当な群構造が検出されることが示されました。

より詳細な解析については、主成分分析（PCA）による高次元データの次元削減解析で確認します。

> 前回: [#11 深層学習結果の前処理](article-11-openms-preprocess.md)
> 次回: [#12b 深層学習DIA PCA解析](article-12b-openms-visualization-pca.md) — 主成分分析による詳細な群分離確認
