---
title: "COSMICデータベースでがん関連タンパク質を特定する【論文再現シリーズ #9】"
emoji: "🎯"
type: "tech"
...
> コアとなる照合ロジックを抜粋します。完全版（遺伝子リスト全体を含む）は Notebook（`notebooks/step_09.ipynb`）を参照してください。
...
| 項目 | 論文 (DIA-NN, 10,329タンパク質) | 本書 (sage, 2,081タンパク質) |
|------|-----------------------------|-----------------------------|
| 全がん関連カバー率 | 531/748 = **71%** | 34/198 = **17.2%** |
| CRC特異的カバー率 | 48/64 = **75%** | 6/65 = **9.2%** |
| 検出された主要ドライバー | - | **KRAS, CTNNB1, CDH1, NRAS, IDH1/2 等 計40個** |

> 備考: 本書の「全がん関連」「CRC特異的」は、スクリプト内で定義した代表的サブセット（COSMIC_ALL_GENES=198個、COSMIC_CRC_GENES=65個）に対するカバー率です。論文は COSMIC Cancer Gene Census 全登録遺伝子を対象としており、母数が異なります（論文: 全がん 748 / CRC 64）。

### COSMICカバレッジの可視化

![COSMICカバレッジ](images/fig_cosmic_coverage.png)

COSMIC Cancer Gene Censusに登録されているがん関連遺伝子のうち、本書のDIA-MSプロテオミクスで同定できた割合を棒グラフで示しています。本書の代表サブセットに対して、全がん関連では198個中34個（17.2%）、CRC特異的では65個中6個（9.2%）をカバーし、CRC・全がん合わせて 40個 のがん関連遺伝子を同定しました（`results/tables/cosmic_overlap.csv`）。

### カバー率が論文より低い理由

| 理由 | 説明 |
|------|------|
| 同定タンパク質数の差 | sage 32ファイル = 2,081 vs DIA-NN 32ファイル = 10,329 |
...
```bash
python scripts/step_08_cosmic_analysis.py
```

**Notebook でセルごとに実行する場合:**

```bash
jupyter notebook notebooks/step_09.ipynb
```

Notebook版では照合結果やカバレッジ図をインラインで確認できます。
